...
BOOL CListCtrlEx::OnNMDblclk(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	CLRow = pNMListView->iItem, CLCol = pNMListView->iSubItem;		

	*pResult = DisplayEditor(CLRow, CLCol);
...
void CListCtrlEx::_setFocus()
{
	try
	{
		CWnd* pWndWithFocus = GetFocus();		
		if(pWndWithFocus == GetDlgItem(DSCV_ED1)) txtE_DMCV.SetSel(0,-1);
		else if(pWndWithFocus == GetDlgItem(DSCV_ED2)) txtE_TCCV.SetSel(0,-1);
		else if(pWndWithFocus == GetDlgItem(DSCV_ED3)) txtE_NDCV.SetSel(0,-1);
		else if(pWndWithFocus == GetDlgItem(DSVL_ED1)) txtE_DMVL.SetSel(0,-1);		
		else if(pWndWithFocus == GetDlgItem(DSVL_ED2)) txtE_TCVL.SetSel(0,-1);		
		else if(pWndWithFocus == GetDlgItem(DSVL_ED3)) txtE_PPVL.SetSel(0,-1);
		else if(pWndWithFocus == GetDlgItem(QCLM_T1)) edTxt.SetSel(0,-1);
		else if(pWndWithFocus == GetDlgItem(QLWB_T1)) qlwb_edit.SetSel(0,-1);
		else if(pWndWithFocus == GetDlgItem(LM3_E1)) edLMN.SetSel(0,-1);
		else if(pWndWithFocus == GetDlgItem(NMSH_E1)) m_editName.SetSel(0,-1);
		else if(pWndWithFocus == GetDlgItem(QLND_E1)) txtE_NKTC.SetSel(0,-1);
		else if(pWndWithFocus == GetDlgItem(CRTD_EDIT)) edCreateTxt.SetSel(0,-1);
	}
	catch(...){}
}
...
BOOL CListCtrlEx::PreTranslateMessage(MSG* pMsg)
{
	...
	CWnd* pWndWithFocus = GetFocus();
	if (pMsg->message == WM_KEYDOWN	&& pMsg->wParam == VK_RETURN && pWndWithFocus == GetDlgItem(DSCV_ED1))
	{
		if(CLCol==1 || CLCol==2 || CLCol==3)
		{
			int iCount = lit_dmcv.GetItemCount();
			if(CLRow+1 == iCount) DisplayEditor(0, CLCol);
			else DisplayEditor(CLRow+1, CLCol);

			return TRUE;
		}
	}
	else if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_RETURN && pWndWithFocus == GetDlgItem(DSVL_ED1))
	{
		if(CLCol==1 || CLCol==2 || CLCol==3)
		{
			int iCount = lit_dmvl.GetItemCount();
			if(CLRow+1 == iCount) DisplayEditor(0, CLCol);
			else DisplayEditor(CLRow+1, CLCol);
		...
		(pMsg->wParam == VK_RETURN || pMsg->wParam == VK_TAB) && pWndWithFocus == GetDlgItem(NMSH_E1))
	{
		if(CLCol==5)
		{
			int iCount = m_listName.GetItemCount();
			if(CLRow+1 == iCount) DisplayEditor(0, 5);
			else DisplayEditor(CLRow+1, 5);

			return TRUE;
		}
	}
	else if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_TAB 
		&& pWndWithFocus == GetDlgItem(DSCV_ED1))
	{
		if (CLCol == 1)
		{
			DisplayEditor(CLRow, 2);
			return TRUE;
		}
		else if (CLCol == 2)
		{
			DisplayEditor(CLRow, 3);
			return TRUE;
		}
		else if (CLCol == 3)
		{
			DisplayEditor(CLRow, 1);
			return TRUE;
		}
	}
	else if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_TAB 
		&& pWndWithFocus == GetDlgItem(DSVL_ED1))
	{
		if (CLCol == 1)
		{
			DisplayEditor(CLRow, 2);
	...
	else if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_RETURN && pWndWithFocus == GetDlgItem(DSCV_ED2))
	{
		if(CLCol==2)
		{
			int iNhay=0;
			CString kt1 = lit_tccv.GetItemText(CLRow,0);
			CString kt2 = lit_tccv.GetItemText(CLRow,CLCol);
			CString val=L"";
			txtE_TCCV.GetWindowTextW(val);
			val.Replace(L"'",L"");
			val.Trim();
			if(kt1==L"1" || val!=kt2)
			{
				...
				iNhay = _dlg.f_Tim_kiem_TC();
			}

			if(iNhay==0)
			{
				int iCount = lit_tccv.GetItemCount();
				if(CLRow+1 == iCount) DisplayEditor(0, CLCol);
				else DisplayEditor(CLRow+1, CLCol);
	...
	else if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_RETURN && pWndWithFocus == GetDlgItem(DSVL_ED2))
	{
		if(CLCol==2)
		{
			int iNhay=0;
			CString kt1 = lit_tcvl.GetItemText(CLRow,0);
			CString kt2 = lit_tcvl.GetItemText(CLRow,CLCol);
			CString val=L"";
			txtE_TCVL.GetWindowTextW(val);
			val.Replace(L"'",L"");
			val.Trim();
			if(kt1==L"1" || val!=kt2)
			{
			...
			if(iNhay==0)
			{
				int iCount = lit_tcvl.GetItemCount();
				if(CLRow+1 == iCount) DisplayEditor(0, CLCol);
				else DisplayEditor(CLRow+1, CLCol);
	...
	else if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_RETURN && pWndWithFocus == GetDlgItem(DSCV_ED3))
	{
		if(CLCol==1 || CLCol==2)
		{
			int iNhay=0;
			CString kt1 = lit_ndkt.GetItemText(CLRow,0);
			CString kt2 = lit_ndkt.GetItemText(CLRow,CLCol);
			CString val=L"";
			txtE_NDCV.GetWindowTextW(val);
			val.Replace(L"'",L"");
			val.Trim();
			if(kt1==L"1" || val!=kt2)
			{
				...
				jTMCV=CLCol;
				xl_tukhoa = val;
				Dlg_noidungppktra _dlg;
				iNhay = _dlg.f_Tim_kiem_NDPP();
			}

			if(iNhay==0)
			{
				int iCount = lit_ndkt.GetItemCount();
				if(CLRow+1 == iCount) DisplayEditor(0, CLCol);
				else DisplayEditor(CLRow+1, CLCol);
	...
	else if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_TAB && pWndWithFocus == GetDlgItem(DSCV_ED3))
	{
		if (CLCol == 1)
		{
			DisplayEditor(CLRow, 2);
	...
	else if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_RETURN && pWndWithFocus == GetDlgItem(DSVL_ED3))
	{
		if(CLCol==2)
		{
			int iNhay=0;
			CString kt1 = lit_ppvl.GetItemText(CLRow,0);
			CString kt2 = lit_ppvl.GetItemText(CLRow,CLCol);
			CString val=L"";
			txtE_PPVL.GetWindowTextW(val);
			val.Replace(L"'",L"");
			val.Trim();
			if(kt1==L"1" || val!=kt2)
			{
				...
				xl_tukhoa = val;
				Dlg_all_mauvl _dlg;
				iNhay = _dlg.f_Tim_kiem_MVL();
			}

			if(iNhay==0)
			{
				int iCount = lit_ppvl.GetItemCount();
				if(CLRow+1 == iCount) DisplayEditor(0, CLCol);
				else DisplayEditor(CLRow+1, CLCol);
		...
		&& pWndWithFocus == GetDlgItem(DSVL_ED3))
	{
		if (CLCol == 2)
		{
			DisplayEditor(CLRow, 3);
	...
	else if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_RETURN && pWndWithFocus == GetDlgItem(QCLM_T1))
	{
		int iRowQLM = listQclm.GetItemCount();
		if(CLRow+1 == iRowQLM)
		{
			if(CLCol==4)
			{
				...
				iChekUp=1;
				CString szval=L"";
				if(iRowQLM<9) szval.Format(L"0%d",iRowQLM+1);
				else szval.Format(L"%d",iRowQLM+1);

				listQclm.InsertItem(iRowQLM,szval,0);
				listQclm.SetItemText(iRowQLM,1,L"[giá trị]");
				listQclm.SetRowColors(iRowQLM,WHITE,BLUE);
				DisplayEditor(iRowQLM,0);
			}
			else DisplayEditor(0, CLCol);
		}
		else DisplayEditor(CLRow+1, CLCol);

		return TRUE;
	}
	else if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_RETURN && pWndWithFocus == GetDlgItem(QLWB_T1))
	{
		if(CLCol==3)
		{
			int iCount = qlwb_list.GetItemCount();
			if(CLRow+1 == iCount) DisplayEditor(0, CLCol);
			else DisplayEditor(CLRow+1, CLCol);
		...
		&& pWndWithFocus == GetDlgItem(QCLM_T1))
	{
		if(CLCol==4) DisplayEditor(CLRow, 0);
		else DisplayEditor(CLRow, CLCol+1);

		return TRUE;
	}
	else if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_RETURN && pWndWithFocus == GetDlgItem(LM3_E1))
	{
		int iCount = litLMN3.GetItemCount();
		...
		if(CLRow+1 == iCount)
		{
			if(CLCol+1==sl)
			{
				// Bổ sung 09.04.2018
				CString szval=L"";
				szval.Format(L"%d",iCount+1);
				litLMN3.InsertItem(iCount,L"",0);
				litLMN3.SetItemText(iCount,1,szval);

				if(sl>=2)
				for (int i = 2; i < sl; i++) litLMN3.SetItemText(iCount,i,L"[giá trị]");
				DisplayEditor(iCount, 1);
			}
			else DisplayEditor(0, CLCol);
		}
		else DisplayEditor(CLRow+1, CLCol);
		...
		&& pWndWithFocus == GetDlgItem(LM3_E1))
	{
		int iCount = litLMN3.GetColumnCount();
		if(CLCol==iCount-1) DisplayEditor(CLRow, 1);
		else DisplayEditor(CLRow, CLCol+1);

		return TRUE;
	}
	else if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_RETURN && pWndWithFocus == GetDlgItem(QLND_E1))
	{
		...
		&& pWndWithFocus == GetDlgItem(QLND_E1))
	{
		if(CLCol==5) DisplayEditor(CLRow,8);
		else if(CLCol==16) DisplayEditor(CLRow,3);
		else DisplayEditor(CLRow, CLCol+1);

		return TRUE;
	}
	else if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_RETURN 
		&& pWndWithFocus == GetDlgItem(CRTD_EDIT))
	{
		int iCount = litCRETD.GetItemCount();
		int sl = litCRETD.GetColumnCount();
		if (CLRow + 1 == iCount)
		{
			if (CLCol + 1 == sl) DisplayEditor(CLRow, 0);
			else DisplayEditor(CLRow + 1, CLCol);
		}
		else DisplayEditor(CLRow + 1, CLCol);
		...
		&& pWndWithFocus == GetDlgItem(CRTD_EDIT))
	{
		int iCount = litCRETD.GetColumnCount();
		if (CLCol == iCount - 1) DisplayEditor(CLRow, 1);
		else DisplayEditor(CLRow, CLCol + 1);
